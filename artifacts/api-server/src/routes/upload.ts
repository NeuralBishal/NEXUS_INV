  ...
  async (req, res): Promise<void> => {
    ...
    try {
      ...
      if (allItems.length === 0) {
        res.status(400).json({
          success: false,
          message: "No inventory items found. Make sure your Excel has a column named Description, Name, or Item.",
          itemsProcessed: 0,
          itemsAdded: 0,
          itemsUpdated: 0,
          itemsRemoved: 0,
      ...
      for (const itemData of allItems) {
        ...
        if (existing) {
          ...
          // Track consumption separately if column exists
          if (consumption && consumption > 0) {
            await db.insert(transactionsTable).values({
              ...
              previousQuantity: String(newQty),
              newQuantity: String(Math.max(0, newQty - consumption)),
              notes: "Consumption recorded from Excel",
            });
          }
          ...
          const [newItem] = await db
            ...
            .values({ ...fields, lastUpdated: new Date() })
            .returning();
      ...
      res.json(UploadExcelResponse.parse({
        success: true,
        message: `Synced ${totalProcessed} items from ${workbook.SheetNames.length} sheet(s) in ${fileName}`,
        itemsProcessed: totalProcessed,
        itemsAdded,
        itemsUpdated,
        itemsRemoved,
      ...
      res.status(400).json({
        success: false,
        message: "Failed to process Excel file: " + String(err),
        itemsProcessed: 0,
        itemsAdded: 0,
        itemsUpdated: 0,
        itemsRemoved: 0,
